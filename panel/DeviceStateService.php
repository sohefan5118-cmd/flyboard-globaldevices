...
class DeviceStateService
{
    ...
    public function setDevices(int $userId, int $nodeId, array $ips): void
    {
        ...
        try {
            ...
            $ips = $this->limitDevices($userId, $reportedIps);
    ...
    public function getUsersDevices(array $userIds): array
    {
        ...
        foreach ($userIds as $userId) {
            ...
            if (!empty($data)) {
                ...
                if (!empty($ips)) {
                    $result[$userId] = $this->limitDevices((int) $userId, array_values(array_unique($ips)));
                }
            }
        }
     ...
     */
    private function limitDevices(int $userId, array $ips): array
    {
        $limit = (int) (User::query()->whereKey($userId)->value('device_limit') ?? 0);
        if ($limit <= 0) {
            return $ips;
        }
        ...
        $allowedGlobal = [];
        foreach ($this->getAllowedDevices($userId) as $ip) {
