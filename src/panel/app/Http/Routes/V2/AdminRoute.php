...
use App\Http\Controllers\V2\Admin\SubscribeGuardController;
...
class AdminRoute
{
    public function map(Registrar $router)
    {
        $router->group([
            ...
            'middleware' => ['admin.ip_whitelist', 'admin', 'log'],
        ], function ($router) {
            // Config
            $router->group([
                'prefix' => 'config'
                ...
                $router->get('/adminIpWhitelistLogs', [ConfigController::class, 'adminIpWhitelistLogs']);
            });
            ...
            // Flyboard subscribe guard
            $router->group([
                'prefix' => 'subscribe-guard'
            ], function ($router) {
                $router->get('/fetch', [SubscribeGuardController::class, 'fetch']);
                $router->post('/drop', [SubscribeGuardController::class, 'drop']);
                $router->post('/clear', [SubscribeGuardController::class, 'clear']);
            });

            // Flyboard upstream subscribe
            $router->group([
                'prefix' => 'upstream-subscribe'
            ], function ($router) {
                $router->get('/profiles', [UpstreamSubscribeController::class, 'profiles']);
                $router->post('/profiles', [UpstreamSubscribeController::class, 'saveProfiles']);
                $router->post('/test', [UpstreamSubscribeController::class, 'test']);
                $router->post('/users', [UpstreamSubscribeController::class, 'users']);
                $router->post('/users/save', [UpstreamSubscribeController::class, 'saveUser']);
                $router->post('/users/delete', [UpstreamSubscribeController::class, 'deleteUser']);
                $router->post('/users/reset-token', [UpstreamSubscribeController::class, 'resetUserToken']);
            });
        });
