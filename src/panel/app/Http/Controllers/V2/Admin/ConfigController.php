...
class ConfigController extends Controller
{
    ...
    private function getConfigMappings(): array
    {
        return [
            ...
            'subscribe' => [
                ...
                'subscribe_guard_enable' => (bool) admin_setting('subscribe_guard_enable', 1),
                'subscribe_guard_window_hours' => (int) admin_setting('subscribe_guard_window_hours', 24),
                'subscribe_guard_max_ips' => (int) admin_setting('subscribe_guard_max_ips', 3),
                'subscribe_guard_action' => admin_setting('subscribe_guard_action', 'reject'),
                'upstream_subscribe_enable' => (bool) admin_setting('upstream_subscribe_enable', 0),
                'upstream_subscribe_url' => admin_setting('upstream_subscribe_url', ''),
                'upstream_subscribe_profiles' => admin_setting('upstream_subscribe_profiles', []),
                'upstream_subscribe_merge_mode' => admin_setting('upstream_subscribe_merge_mode', 'append'),
                'upstream_subscribe_timeout' => (int) admin_setting('upstream_subscribe_timeout', 10),
                'upstream_subscribe_max_bytes' => (int) admin_setting('upstream_subscribe_max_bytes', 524288),
                'upstream_subscribe_user_agent' => admin_setting('upstream_subscribe_user_agent', ''),
            ],
            'frontend' => [
            ...
            'safe' => [
                ...
                'admin_ip_whitelist_enable' => (bool) admin_setting('admin_ip_whitelist_enable', 0),
                'admin_ip_whitelist_entries' => admin_setting('admin_ip_whitelist_entries', []),
                'admin_ip_whitelist_note' => admin_setting('admin_ip_whitelist_note', ''),
                // 保持向后兼容
    ...
    public function adminIpWhitelistLogs(Request $request)
    {
        $perPage = min(max((int) $request->input('per_page', 20), 1), 100);
        $query = DB::table('v2_log')
            ->where('title', 'Admin IP whitelist blocked request')
            ->orderByDesc('id');
        ...
        $logs->getCollection()->transform(function ($row) {
            $context = json_decode((string) ($row->context ?: $row->data), true) ?: [];
            return [
                'id' => $row->id,
                'ip' => $row->ip,
                'uri' => $row->uri,
