...
class ServerService
{
    ...
    public static function getAvailableServers(User $user): array
    {
        ...
        $servers = collect($servers)->map(function ($server) use ($user) {
            ...
            $server->password = app(NodeAuthAliasService::class)->serverPassword($server, $user);
    ...
    public static function getAvailableUsers(Server $node)
    {
        ...
        if ((int) admin_setting('subscribe_guard_enable', 1) === 1) {
            $auth = app(NodeAuthAliasService::class);
            $users = $users->map(fn ($user) => $auth->userForNode($user));
        }
    ...
    public static function processTraffic(Server $node, array $traffic): void
    {
        ...
        $node->rate = $node->getCurrentRate();
        $changedUpstreamUserIds = $upstream->processNodeTraffic($data, $node->rate);
        if (!empty($changedUpstreamUserIds)) {
            NodeSyncService::notifyUpstreamUsersUpdated();
        }

        $mainUserData = array_filter(
            $data,
            fn ($value, $uid) => !$upstream->isNodeUserId((int) $uid),
            ARRAY_FILTER_USE_BOTH
        );

        if (!empty($mainUserData)) {
            (new UserService())->trafficFetch($node, $node->type, $mainUserData);
        }
    }
    ...
    {
        app(DeviceStateService::class)->syncNodeDevices($nodeId, $alive);
    }
