...
Route::get('/', function (Request $request) {
    ...
    try {
        ...
        $renderParams = [
            ...
            'description' => admin_setting('app_description', 'Flyboard is best'),
...
Route::get('/' . admin_setting('secure_path', admin_setting('frontend_admin_path', hash('crc32b', config('app.key')))) . '/flyboard', function () {
    return view('flyboard-admin', [
        'title' => admin_setting('app_name', 'Flyboard') . ' - Flyboard 管理',
        'secure_path' => admin_setting('secure_path', admin_setting('frontend_admin_path', hash('crc32b', config('app.key')))),
...
Route::get('/upstream-s/{token}/qr.svg', [\App\Http\Controllers\V2\Admin\UpstreamSubscribeController::class, 'qr'])
    ->name('flyboard.upstream.qr');

Route::get('/upstream-u/{token}', [\App\Http\Controllers\V2\Admin\UpstreamSubscribeController::class, 'userPage'])
    ->name('flyboard.upstream.user_page');

Route::get('/upstream-s/{token}', [\App\Http\Controllers\V2\Admin\UpstreamSubscribeController::class, 'subscribe'])
    ->name('flyboard.upstream.subscribe');
