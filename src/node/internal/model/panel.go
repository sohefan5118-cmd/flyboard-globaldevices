...
func UserSpecsFromPanel(users []panel.User) []UserSpec {
	...
	for _, user := range users {
		out = append(out, UserSpec{ID: user.ID, UUID: user.UUID, AuthAliases: cloneStringSlice(user.AuthAliases), ExpiredAt: user.ExpiredAt, SpeedLimit: user.SpeedLimit, DeviceLimit: user.DeviceLimit})
	}
	return out
}
...
func UserSpecsToPanel(users []UserSpec) []panel.User {
	...
	for _, user := range users {
		out = append(out, panel.User{ID: user.ID, UUID: user.UUID, AuthAliases: cloneStringSlice(user.AuthAliases), ExpiredAt: user.ExpiredAt, SpeedLimit: user.SpeedLimit, DeviceLimit: user.DeviceLimit})
	}
	return out
}
