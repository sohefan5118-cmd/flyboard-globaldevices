...
func (w *WSClient) handleDataEvent(msg wsMessage) {
	...
	case WSEventSyncUsers:
		...
		// sync.users is an authoritative full snapshot. An empty list is valid
		// and means the node must remove all currently loaded users, e.g. when
		// the last user expires, is banned, or runs out of traffic. Do not drop it
		// as a malformed payload, otherwise stale users can keep using the node.
		event.Users = p.Users
		event.NodeID = p.NodeID
