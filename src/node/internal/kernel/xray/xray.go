...
func (x *Xray) CloseUserConnections(_ context.Context, uuid string) error {
	x.mu.Lock()
	ld := x.limitDispatcher
	users := append([]model.UserSpec(nil), x.users...)
	x.mu.Unlock()
	if ld == nil || uuid == "" {
		return nil
	}
	...
	for _, u := range users {
		for _, auth := range u.AuthUUIDs() {
			if auth == uuid {
				emails[userAuthEmail(u, auth)] = struct{}{}
			}
		}
	}
	closed := ld.CloseUsers(emails)
	if closed > 0 {
		nlog.Core().Info("xray: closed user connections", "uuid", uuid, "connections", closed)
	}
	return nil
}
...
// UpdateGlobalDevices updates xray's dispatcher with the panel-selected global
// device allowlist. Without this, each newly added node admitted its own local
// first device, multiplying a user's device_limit by node count.
...
// ClearGlobalDevices clears xray's dispatcher-side global allowlist.
...
// RemoveUsers removes users from the running kernel.
// For xray we must force a replacement instance so removed users' existing
...
func (x *Xray) RemoveUsers(users []model.UserSpec) (int, error) {
	...
	if len(kept) == 0 {
		x.forceCloseRunningInstance()
		nlog.Core().Info("xray: users removed, kernel stopped", "removed", removed)
		return removed, nil
	}

	nlog.Core().Info("xray: users removed, restarting to drop existing connections", "removed", removed, "total", len(kept))
	if err := x.restartWithUsers(nc, kept, t); err != nil {
		return 0, err
	}
	return removed, nil
}
...
func (x *Xray) UpdateUsers(users []model.UserSpec) (added, removed int, err error) {
	...
	if removed > 0 {
		nc, t := x.nodeConfig, x.tls
		x.mu.Unlock()
		nlog.Core().Info("xray: user removal detected, restarting to drop stale connections", "added", added, "removed", removed, "total", len(users))
		if err = x.restartWithUsers(nc, users, t); err != nil {
			return 0, 0, err
		}
...
// forceCloseRunningInstance stops the current instance immediately, without
// the graceful drain used by Stop()/closeOld(). This is required when user
// removal must invalidate existing sessions right away.
func (x *Xray) forceCloseRunningInstance() {
	x.mu.Lock()
	inst := x.instance
	ld := x.limitDispatcher
	x.instance = nil
...
// restartWithUsers replaces the running xray instance immediately so the old
// user's connections are terminated before the replacement starts accepting.
func (x *Xray) restartWithUsers(nodeConfig *model.NodeSpec, users []model.UserSpec, tls kernel.TLSCert) error {
	x.forceCloseRunningInstance()
	if len(users) == 0 {
		return nil
	}
...
func (x *Xray) updateDispatcherLimits(users []model.UserSpec) {
	...
	emailToDevice := make(map[string]string, len(users)*3)
	...
	for _, u := range users {
		for _, auth := range u.AuthUUIDs() {
			...
			emailToDevice[email] = auth
			emailToDevice[auth] = auth
			if u.DeviceLimit > 0 {
	...
	ld.UpdateLimits(emailToUID, deviceLimits, emailToDevice)
}
