...
func TestXrayGetUserTrafficUsesBuiltInStatsAndDispatcherState(t *testing.T) {
	...
	ld.mu.Lock()
	ld.limitedIPs[email1] = map[string]int{"device-1": 1}
	ld.mu.Unlock()
	ref := &atomic.Int64{}
	ref.Store(1)
	ic := &deviceMapCounter{}
	ic.ips.Store("device-2", ref)
	...
	traffic, aliveDevices, connCount, err := x.GetUserTraffic(context.Background())
	if err != nil {
	...
	if !aliveDevices[1]["device-1"] {
		t.Fatal("expected limited-user credential to be reported")
	}
	if !aliveDevices[2]["device-2"] {
		t.Fatal("expected unlimited-user credential to be reported")
	}
