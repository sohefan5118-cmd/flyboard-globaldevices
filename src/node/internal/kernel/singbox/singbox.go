...
// RemoveUsers hot-swaps users out of running inbounds and force-closes removed
// users' existing connections.
func (s *SingBox) RemoveUsers(users []model.UserSpec) (int, error) {
	...
	var removedUsers []model.UserSpec
	removed := 0
	for _, u := range s.users {
	...
	s.closeRemovedUsersLocked(removedUsers)
	if len(kept) == 0 {
		s.stop()
		return removed, nil
	}
...
// UpdateUsers replaces the entire user set atomically via hot-swap. Removed
// users' existing connections are force-closed first.
...
// closeRemovedUsersLocked force-closes existing connections for removed users.
// Must be called with s.mu held.
func (s *SingBox) closeRemovedUsersLocked(users []model.UserSpec) {
	if s.connTracker == nil {
		return
	}
	for _, u := range users {
		for _, auth := range u.AuthUUIDs() {
			s.connTracker.CloseByUUID(auth)
		}
	}
}
