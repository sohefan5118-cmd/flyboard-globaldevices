...
type Service struct {
	...
	expireTimer      *time.Timer                    // next scheduled expiration sweep
...
type pullResult struct {
	...
	configHash         string
	userHash           string
	configBaselineHash string
	userBaselineHash   string
	certChanged        bool
}
...
func (s *Service) initialSetup(ctx context.Context) error {
	...
	bootstrap.Users = activeUsersAt(bootstrap.Users, time.Now())
...
func (s *Service) handleWSEvent(ctx context.Context, event controlplane.Event) {
	...
	case controlplane.EventSyncUsers:
		...
		event.Users = activeUsersAt(event.Users, time.Now())
...
func (s *Service) pullViaAPIAsync(ctx context.Context) {
	...
	currentUserHash := s.lastUserHash
	...
	go func() {
		...
		if snapshot.Config != nil {
			...
			result.configBaselineHash = currentConfigHash
			if result.configHash == currentConfigHash && !certChanged {
				result.config = nil
			}
		}
		if snapshot.Users != nil {
			result.users = activeUsersAt(snapshot.Users, time.Now())
			result.userHash = computeUserHash(result.users)
			result.userBaselineHash = currentUserHash
		}
...
func (s *Service) applyPullResult(ctx context.Context, result pullResult) {
	...
	configChanged := false
	if result.config != nil && result.configBaselineHash != "" && result.configBaselineHash != s.lastConfigHash {
		nlog.Core().Debug("dropping stale polled config snapshot", "baseline", result.configBaselineHash, "current", s.lastConfigHash)
		result.config = nil
	}
	if result.users != nil && result.userBaselineHash != "" && result.userBaselineHash != s.lastUserHash {
		nlog.Core().Debug("dropping stale polled users snapshot", "baseline", result.userBaselineHash, "current", s.lastUserHash)
		result.users = nil
	}
...
func (s *Service) updateUserState(users []model.UserSpec) {
	users = activeUsersAt(users, time.Now())
	if users == nil {
		users = []model.UserSpec{}
	}
...
func (s *Service) applyUserDelta(ctx context.Context, action string, deltaUsers []model.UserSpec) {
	switch action {
	case "add":
		deltaUsers = activeUsersAt(deltaUsers, time.Now())
...
func activeUsersAt(users []model.UserSpec, now time.Time) []model.UserSpec {
	if users == nil {
		return nil
	}
	out := users[:0]
	for _, u := range users {
		if u.IsExpiredAt(now) {
			continue
		}
		out = append(out, u)
	}
...
func (s *Service) expireUsers(ctx context.Context, now time.Time) {
	s.metricsMu.RLock()
	users := append([]model.UserSpec(nil), s.lastUsers...)
	s.metricsMu.RUnlock()

	active := activeUsersAt(users, now)
	if len(active) == len(users) {
		return
	}
	expired := len(users) - len(active)
	if s.nodeLog != nil {
		s.nodeLog.Info(fmt.Sprintf("users expired: -%d", expired))
	}
	if s.kernel.IsRunning() {
		s.applyUserUpdate(ctx, active, computeUserHash(active))
		return
	}
	s.updateUserState(active)
}

func (s *Service) scheduleNextExpire(now time.Time) {
	if s.expireTimer != nil {
		if !s.expireTimer.Stop() {
			select {
	...
	next, ok := nextExpiryAt(s.lastUsers, now)
	if !ok {
		return
	}
	d := time.Until(next)
	...
	s.expireTimer = time.NewTimer(d)
}

func (s *Service) stopExpireTimer() {
	if s.expireTimer == nil {
		return
	}
	if !s.expireTimer.Stop() {
...
func nextExpiryAt(users []model.UserSpec, now time.Time) (time.Time, bool) {
	var next time.Time
	found := false
	for _, u := range users {
		if u.ExpiredAt <= 0 {
		...
		expiry := time.Unix(u.ExpiredAt, 0)
		if !now.Before(expiry) {
			return now, true
		}
		if !found || expiry.Before(next) {
			next = expiry
			found = true
		}
	}
...
func computeUserHash(users []model.UserSpec) string {
	...
	for _, u := range sorted {
		...
		binary.LittleEndian.PutUint64(buf[:], uint64(u.ExpiredAt))
		h.Write(buf[:])
		for _, alias := range u.AuthAliases {
			io.WriteString(h, alias)
			h.Write([]byte{0})
		}
